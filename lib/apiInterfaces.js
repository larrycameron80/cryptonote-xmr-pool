...
function rpcOther(host, port, method, uri, callback){
    jsonHttpRequest(host, port, false, function(error, replyJson){
        if (error){
            callback(error);
            return;
        }
        callback(replyJson.status == "OK" ? null : replyJson.status, replyJson)
    }, '/' + uri);
}
...
module.exports = function(daemonConfig, walletConfig, poolApiConfig){
    return {
        ...
        rpcOther: function(uri, callback) {
            rpcOther(daemonConfig.host, daemonConfig.port, 'post', uri, callback);
        },
